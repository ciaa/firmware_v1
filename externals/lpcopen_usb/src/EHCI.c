...
#include "HCD/HCD.h"
#include "HCD/EHCI/EHCI.h"


/* User code */
extern void _usb_host_on_connection(uint8_t HostID);
extern void _usb_host_on_disconnection(uint8_t HostID);
...
//
//	status = (HCD_STATUS) HcdQHD(HostID, HeadIdx)->status;
//	if (status == HCD_STATUS_TRANSFER_Stall)
//	{
//		HcdQHD(HostID, HeadIdx)->
//	}
//
	return HCD_STATUS_OK;
}
...
HCD_STATUS HcdControlTransfer(uint32_t PipeHandle,
							  const uint8_t* const pDeviceRequest,
							  uint8_t *const buffer)
{
	...
	if (pDeviceRequest == NULL)
		ASSERT_STATUS_OK_MESSAGE(HCD_STATUS_PARAMETER_INVALID, "Device Request is NULL");

	Datalength = ((uint16_t*) pDeviceRequest)[3]; /* wLength */
 	if  (Datalength && buffer == NULL)
		ASSERT_STATUS_OK_MESSAGE(HCD_STATUS_PARAMETER_INVALID, "Data stage required byt Data Buffer is NULL");

	ASSERT_STATUS_OK(PipehandleParse(PipeHandle, &HostID, &XferType, &QhdIdx) );

	direction  =  pDeviceRequest[0] & 0x80; /* bmRequestType */
						   ...
						   uint32_t *pQhdIdx)
{
	...
	HcdQHD(HostID,
		   *pQhdIdx)->uFrameSMask =
		(TransferType == INTERRUPT_TRANSFER) ? (DeviceSpeed == HIGH_SPEED ? 0xFF : Interval) : 0;
...
static HCD_STATUS PortStatusChangeIsr(uint8_t HostID, uint32_t deviceConnect)
{
	if (deviceConnect) /* Device Attached */
		_usb_host_on_connection(HostID);
	else /* Device detached */
		_usb_host_on_disconnection(HostID);
	return HCD_STATUS_OK;
}
