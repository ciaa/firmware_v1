...
{                      
	...
	scu_pinmux(0x8, 2, MD_PDN, FUNC0); // PE_9 = GPIO7[9]  
    LPC_GPIO_PORT->DIR[4] |= 1 << 2;   // output
    LPC_GPIO_PORT->CLR[4] |= 1 << 2;   // set to LOW, the reset input of the UDA1380 is high active
  

    // Hardware reset to the UDA1380
    delay (10000);
    LPC_GPIO_PORT->SET[4] |= 1 << 2;   // set to HIGH
    delay (10000);
    LPC_GPIO_PORT->CLR[4] |= 1 << 2;   // set to LOW
    delay (10000); 
